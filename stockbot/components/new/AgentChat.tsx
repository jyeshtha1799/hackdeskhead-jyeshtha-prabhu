...
const AgentChat = ({ agent, messages, isCollapsed, onToggleCollapse, unreadCount, handlers}) => {
  const [input, setInput] = useState('')

  const handleSend = () => {
    if (input.trim()) {
      if (agent === 'Risk Manager') {
        handleRiskOrFollowUp();
      }
      // onSendMessage(input);
      setInput('');
    }
  ...
  const handleRiskOrFollowUp = () => {
    console.log('messages', messages)
    const { onStartRiskAnalysis, onFollowUpQuestion } = handlers[agent] || {};
    if (messages.length === 0 && onStartRiskAnalysis) {
      onStartRiskAnalysis(input);
    } else if (onFollowUpQuestion) {
      onFollowUpQuestion(input);
    }
  }
