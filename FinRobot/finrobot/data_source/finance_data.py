...
from .Historical_Stock_Event_Storage import HistoricalStockEventStorage
...
def get_data(
        ...
        num_chunks:int = 1,
        populate_historical_events: bool = False,
        historical_start_date: str = None,
        historical_end_date: str = None,
):
    ...
    if populate_historical_events:
        if not (historical_start_date and historical_end_date):
            raise ValueError("Both historical_start_date and historical_end_date must be provided to populate historical events.")
        event_storage = HistoricalStockEventStorage()
        event_storage.populate_historical_events(ticker, historical_start_date, historical_end_date)
        historical_events = event_storage.get_events(ticker, start_date=f"{year}-01-01", end_date=f"{year}-12-31")
        event_storage.close()
        result_data['historical_events'] = historical_events
